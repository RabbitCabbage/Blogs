...
OCaml's toplevel is called *utop* as a **calculator+command-line interface** (utop is so luxury, even with tab completion and syntax highlighting), some other languages call the toplevel a REPL. Like JShell for Java and interactive interpreter for Python. It's working logic is Read-Eval-Print Loop.
Typically when we write OCaml in a `.ml` file we don't need `;;`. It's only needed when we write in a interactive session like utop. The toplevel reads until hitting one `;;` to know that OK this is one chunk of input (***a toplevel phrase***). So `;;` means where phrase is complete and ready for parsing and evaluation.
```ocaml
...
> This is the _tail-call optimization_. It can even be applied in cases beyond recursive functions if <font color="#f79646">the calling function’s stack frame is suitably compatible with the callee.</font>
...
***Patterns***: A constant matches itself. <font color="#f79646">An identifier matches anything and binds itself (the name) with the matched value in the scope of the branch. </font>The underscore matches anything and doesn't bind.
...
And the compiler also tests ***exhaustiveness/exhaustivity*** and ***unused branches***.
```ocaml
let bad_test lst =
    match lst with
    | [] -> "empty";;
